...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, rpi, mask=None):
        ...
        relative_position_bias = self.relative_position_bias_table[rpi.view(-1)].view(
            self.window_size[0] * self.window_size[1]* self.window_size[2], self.window_size[0] * self.window_size[1]* self.window_size[2], -1)  # Wh*Ww*Wd,Wh*Ww*Wd,nH
...
class HAB(nn.Module):
    ...
    def forward(self, x, x_size, rpi_sa, attn_mask):
        ...
        conv_x = conv_x.permute(0, 2, 3,4, 1).contiguous().view(b, h * w * d, c)
        ...
        # partition windows
        x_windows = window_partition(shifted_x, self.window_size)  # nw*b, window_size, window_size,self.window_size, c
        x_windows = x_windows.view(-1, self.window_size * self.window_size* self.window_size, c)  # nw*b, window_size*window_size* self.window_size, c
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = x[:, 0::2, 1::2,0::2, :]  
        x3 = x[:, 0::2, 1::2,1::2, :]  
        x4 = x[:, 1::2, 0::2,0::2 ,:]  # b h/2 w/2 c
        x5 = x[:, 1::2, 0::2, 1::2, :]  
        x6 = x[:, 1::2, 1::2,0::2, :]  
        x7 = x[:, 1::2, 1::2,1::2, :]  
        x = torch.cat([x0, x1, x2, x3, x4, x5, x6, x7], -1)  # b h/2 w/2 8*c
...
class OCAB(nn.Module):
    ...
    def __init__(self, dim,
        ...
        # define a parameter table of relative position bias
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((window_size + self.overlap_win_size - 1) * (window_size + self.overlap_win_size - 1) * (window_size + self.overlap_win_size - 1), num_heads))  # 2*Wh-1 * 2*Ww-1* 2*Wd-1, nH
...
class HAT_3D(nn.Module):
    ...
    def calculate_rpi_sa(self):
        ...
        coords = torch.stack(torch.meshgrid([coords_h, coords_w, coords_d]))  # 2, Wh, Ww, Wd
        coords_flatten = torch.flatten(coords, 1)  # 2, Wh*Ww*Wd
        relative_coords = coords_flatten[:, :, None] - coords_flatten[:, None, :]  # 2, Wh*Ww*Wd, Wh*Ww*Wd
        relative_coords = relative_coords.permute(1, 2, 0).contiguous()  # Wh*Ww*Wd, Wh*Ww*Wd, 2
        relative_coords[:, :, 0] += self.window_size - 1  # shift to start from 0
        relative_coords[:, :, 1] += self.window_size - 1
        relative_coords[:, :, 0] *= 2 * self.window_size - 1
        relative_position_index = relative_coords.sum(-1)  # Wh*Ww, Wh*Ww,*Wd
    ...
    def calculate_rpi_oca(self):
        ...
        coords_ori = torch.stack(torch.meshgrid([coords_h, coords_w, coords_d]))  # 2, ws, ws,ws
        coords_ori_flatten = torch.flatten(coords_ori, 1)  # 2, ws*ws*ws

        coords_h = torch.arange(window_size_ext)
        coords_w = torch.arange(window_size_ext)
        coords_d = torch.arange(window_size_ext)
        coords_ext = torch.stack(torch.meshgrid([coords_h, coords_w, coords_d]))  # 2, wse, wse,wse
        coords_ext_flatten = torch.flatten(coords_ext, 1)  # 2, wse*wse*wse

        relative_coords = coords_ext_flatten[:, None, :] - coords_ori_flatten[:, :, None]   # 2, ws*ws*ws, wse*wse*wse

        relative_coords = relative_coords.permute(1, 2, 0).contiguous()  # ws*ws*ws, wse*wse*wse, 2
    ...
    def calculate_mask(self, x_size):
        ...
        w_slices = (slice(0, -self.window_size), slice(-self.window_size,
                                                       -self.shift_size), slice(-self.shift_size, None))
        d_slices = (slice(0, -self.window_size), slice(-self.window_size,
                                                       -self.shift_size), slice(-self.shift_size, None))
        cnt = 0
        for h in h_slices:
            for w in w_slices:
